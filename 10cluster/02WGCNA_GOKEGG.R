...
info <- read.csv('results/WGCNA/green_module_gene.csv', row.names = 1)
...
#gene ID转换
gene <- bitr(info$x,fromType = 'SYMBOL',toType = 'ENTREZID',OrgDb = GO_database)
write.csv(gene, 'results/WGCNA/green_gene_to_ENTREZID.csv', row.names = F)
...
GO<-enrichGO(gene$ENTREZID,#GO富集分析
             ...
             readable = T)
write.csv(GO, 'results/WGCNA/green_GO.csv', row.names = F)

##气泡图
GO_dataset <- read.csv('results/WGCNA/green_GO.csv')
...
KEGGplot <- ggplot(GOBP_dataset, aes(y = order, x = GeneRatio)) +
  ...
  labs(color = expression(pvalue), size = "Count",
       x = "Gene Ratio", y = "Pathways", title = "GOBP Pathway") +
  theme_bw() +
  theme(axis.text = element_text(size = 14, color = "black"),  # 调整坐标轴刻度字体大小和颜色
        axis.title = element_text(size = 14, color = "black"),  # 调整坐标轴标题字体大小和颜色
...
print(KEGGplot)
# 保存为 PDF 文件
ggsave("results/WGCNA/Green_GOBP_pathway_enrichment.pdf", plot = KEGGplot, width = 6, height = 6)
...
KEGGplot <- ggplot(GOBP_dataset, aes(y = order, x = GeneRatio)) +
  ...
  scale_color_gradient(low = "red", high = "blue") +
  labs(color = expression(pvalue), size = "Count",
       x = "Gene Ratio", y = "Pathways", title = "GOCC Pathway") +
  theme_bw() +
  theme(axis.text = element_text(size = 14, color = "black"),  # 调整坐标轴刻度字体大小和颜色
        axis.title = element_text(size = 14, color = "black"),  # 调整坐标轴标题字体大小和颜色
        plot.title = element_text(size = 16, color = "black")) +  # 调整标题字体大小和颜色
...
print(KEGGplot)
# 保存为 PDF 文件
ggsave("results/WGCNA/Green_GOCC_pathway_enrichment.pdf", plot = KEGGplot, width = 6, height = 6)
...
KEGGplot <- ggplot(GOBP_dataset, aes(y = order, x = GeneRatio)) +
  ...
  scale_color_gradient(low = "red", high = "blue") +
  labs(color = expression(pvalue), size = "Count",
       x = "Gene Ratio", y = "Pathways", title = "GOMF Pathway") +
  theme_bw() +
  theme(axis.text = element_text(size = 14, color = "black"),  # 调整坐标轴刻度字体大小和颜色
        axis.title = element_text(size = 14, color = "black"),  # 调整坐标轴标题字体大小和颜色
        plot.title = element_text(size = 16, color = "black")) +  # 调整标题字体大小和颜色
...
print(KEGGplot)
# 保存为 PDF 文件
ggsave("results/WGCNA/Green_GOMF_pathway_enrichment.pdf", plot = KEGGplot, width = 6, height = 6)
...
#                  pvalueCutoff = 0.05,
#                  qvalueCutoff = 0.05)
# write.csv(KEGG, 'results/WGCNA/Green_KEGG.csv', row.names = F)
# ##气泡图
# KEGG_dataset <- read.csv('results/WGCNA/Green_KEGG.csv')
# # 假设 GeneRatio 列的格式是 "a/b"
...
#   theme_bw() +
#   theme(axis.text = element_text(size = 12, color = "black"),  # 调整坐标轴刻度字体大小和颜色
#         axis.title = element_text(size = 14, color = "black"),  # 调整坐标轴标题字体大小和颜色
#         plot.title = element_text(size = 16, color = "black")) +  # 调整标题字体大小和颜色
#   scale_y_discrete(labels = function(x) str_wrap(x, width = 30))  # 设置纵轴标签换行，宽度为20个字符
# 
...
# print(KEGGplot)
# # 保存为 PDF 文件
# ggsave("results/WGCNA/Green_KEGG_pathway_enrichment.pdf", plot = KEGGplot, width = 6, height = 6)
