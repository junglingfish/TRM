...
library(parallel)            #安装并加载所需R包
library(dplyr)
library(tibble)


## #处理exp矩阵
# exp <- read.table(paste0(routine_result, 'tumor_expr_batch.txt'), check.names = F)
exp <- read.table(paste0(routine_data, 'GEO/GSE53625/GSE53625_tumor_count_loged.txt'), check.names = F)
# 进行反 log2 处理
exp_log2 <- 2^exp

write.table(exp_log2, 'results/geo_tumor_expr_batch.txt', sep="\t", quote = F)


source("Cibersort.R") # 加载R脚本
result1 <- CIBERSORT("LM22.txt", 'results/geo_tumor_expr_batch.txt', perm = 1000, QN = T)        #运行R脚本进行CIBERSORT分析；
...
# 筛选符合条件的行
a <- a[keep_rows, ]
write.csv(a, 'results/geo_cibersort_result.csv')
...
a <- read.csv("results/geo_cibersort_result.csv", row.names = 1)
...
write.csv(result, 'results/cibersort_group_result.csv')

result <- read.csv('results/cibersort_group_result.csv', row.names = 1)
...
# 将图形保存为 PDF 文件
pdf("results/Cibersort.pdf", width = 12, height = 8)  # 调整 width 和 height 根据需要更改图形尺寸
