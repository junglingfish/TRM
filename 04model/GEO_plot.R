...
library(ggplot2)
library(reshape2)
library(gridExtra) # 用于将多个图放在一张图中

# 读取数据
# riskScore_cli <- read.csv(paste0(routine_fig3, 'GEO_riskscore_category_WGCNA.csv'))
riskScore_cli <- read.delim('results/gse53625.risk.txt', sep='\t',header = T,check.names = F)
...
roc_plot <- recordPlot()
# 打开 PDF 设备
pdf("results/GEO_ROCplot.pdf", width = 6, height = 5.25)
# 重放已记录的绘图
replayPlot(roc_plot)
# 关闭 PDF 设备
dev.off()
...
ggsave(filename = 'results/GEO_KMplot.pdf', plot = g2, width = 6, height = 5.25, units = "in")
...
ggsave(filename = 'results/GEO_riskplot.pdf', plot = p3, width = 6, height = 8, units = "in")
